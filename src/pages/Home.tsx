...
export default function Home({ lang }: { lang: "en" | "pt" }) {
  ...
  return (
    <section
      id="home"
      className="transition-colors flex flex-col items-center justify-center md:grid md:grid-cols-2 gap-4 h-screen bg-slate-200 dark:bg-midgray dark:text-verylightgreen text-black"
    >
      <section className="w-full md:h-full flex flex-col md:justify-center items-center">
        <img
          className="max-w-100 max-h-100 w-7/12 md:w-2/3 rounded-[100rem] shadow-lg  shadow-green-500 dark:shadow-green-800 hover:rotate-12 hover:scale-105 transition-transform duration-500"
          src="https://avatars.githubusercontent.com/u/66537057?v=4"
        />
      </section>
      <section className="w-full flex flex-col justify-center md:text-2xl">
        <section className="w-full flex flex-col justify-center gap-3 items-center md:items-start">
          <h1 className="w-full text-xl md:text-3xl flex flex-row justify-center items-center md:justify-start md:flex-row ">
          ...
          <section className="">
            <span>{"<"}</span>
            <span className="text-green-500">{`${subTitle.split(" ")[0]}`}</span>
          ...
          <span className="flex gap-3">
            <a href="https://github.com/luizgustavojunqueira" target="_blank">
              <svg
                ...
                viewBox="0 0 50 50"
                className="w-12 h-12 fill-current dark:hover:text-green-200 hover:text-green-500"
              >
